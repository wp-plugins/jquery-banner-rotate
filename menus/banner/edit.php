...
<form action="" method="post" style="width:490px;" onsubmit="return valida(this);">
	...
	<input type="hidden" name="hash_id" id="hash_id" value="<?= jbr_hash_value($banner->id); ?>" />
	<input type="hidden" name="action" id="action" value="update" />
	<input type="hidden" name="hash_action" id="hash_action" value="<?= jbr_hash_value('update') ?>" />
	<h2><?php jbr_the_translate('Edit'); ?></h2>
	<table class="wp-list-table widefat fixed">
		...
		<tr>
			...
			<td>
				<input type="text" name="dataRetirada" id="dataRetirada" placeholder="dd/mm/yyyy"
					value="<?= ($banner->data_retirada != '')? jbr_date($banner->data_retirada) : "" ?>">
			</td>
		</tr>
		<tr>
			<td>
			...
			<td>
				<input type="text" name="pagina" id="pagina" value="<?= $banner->pagina; ?>">
			</td>
		</tr>
		<tr>
				...
				<select name="slider_id">
					...
					<?php foreach($sliders as $slider) : ?>
						<option value="<?= $slider->id ?>"
							<?php if($banner->slider_id == $slider->id) echo 'selected' ?>>
							<?= $slider->nome ?>
						</option>
					<?php endforeach; ?>
				</select>
		...
		<tr>
			...
			<td>
				<input type="checkbox" name="nova" id="nova" value="1" <?= ($banner->nova == '1')? 'checked': ''?>>
			</td>
		</tr>
		<tr>
			<td>
				<input class="button-primary" type="submit" id="button" value="<?php jbr_the_translate('Update'); ?>">
			</td>
		</tr>
	</table>
	<br/>
